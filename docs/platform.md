...
- [x] ~~CF strategy modules communautaires~~ : v0 = **DSL + Firestore lecture only** pour la communauté (zéro CF tiers, capabilities limitées à `firestore.read` / `module.read`). Les officiels gardent leur CF Python déployée par l'équipe core dans le projet Firebase de la commune. Roadmap : option **B** (module héberge ses propres CF, capability explicite `cf.external` envoyant l'ID token de la commune) pour le jour où un module communautaire a besoin de data externe (météo, iCal, scrapers). Validé 2026-04-30.
- [x] ~~Source du tenant config~~ : migration de `tenants/<id>/app.json` (versionné dans le repo plateforme) vers **Firestore-backed** (doc `_config/modules` dans le projet de chaque commune). Le preloader iOS/Android fetch ça au boot **avant** de précharger les modules. Le dashboard admin écrit dedans. Le JSON versionné devient juste un seed initial pour bootstrapper un nouveau projet commune. Sans ça, impossible d'activer un module communautaire sans une PR sur le core. Validé 2026-04-30.
- [x] ~~Modèle de permissions~~ : **statique** en v0 — manifest = contrat figé. À l'activation depuis le dashboard, la commune voit la liste des capabilities (style Android permissions) et accepte tout-ou-rien. Pas de granularité (genre "désactiver `device.location` mais garder le reste"). Tiered (device permissions dynamiques, data sources statiques) plus tard si un cas concret le demande. Validé 2026-04-30.
